...
// ── Export Format ──
// .COSMOS files are JSON with this structure.
 ...
 * Format: COSMOS:task:v1:<base64>
 */
export function encodeForClipboard(data: PilosExportFile): string {
  const json = JSON.stringify(data)
  const base64 = btoa(unescape(encodeURIComponent(json)))
  return `COSMOS:task:v1:${base64}`
}

// ── Import ──

/**
 * Parses and validates a .COSMOS JSON string.
...
export function parseExportFile(jsonString: string): PilosExportFile {
  ...
  // Validate envelope
  const COSMOS = obj.COSMOS as Record<string, unknown> | undefined
  if (!COSMOS || typeof COSMOS !== 'object') {
    throw new Error('Not a COSMOS export file — missing header')
  }
  if (COSMOS.type !== 'task') {
    throw new Error(`Unsupported export type: ${COSMOS.type}`)
  }
  if (typeof COSMOS.version === 'number' && COSMOS.version > 1) {
    throw new Error('This file was created with a newer version of COSMOS. Please update the app.')
  }
  if (COSMOS.version !== 1) {
 ...
 * Validates: starts with 'COSMOS:task:v1:', base64 decodes, then calls parseExportFile.
 */
export function decodeFromClipboard(clipboardText: string): PilosExportFile {
  const trimmed = clipboardText.trim()
  if (!isPilosClipboardString(trimmed)) {
    throw new Error('Not a valid COSMOS clipboard payload')
  }

  const base64 = trimmed.slice('COSMOS:task:v1:'.length)
  let json: string
  try {
...
/**
 * Lightweight check: does the string look like a COSMOS clipboard payload?
 */
export function isPilosClipboardString(text: string): boolean {
  return text.trim().startsWith('COSMOS:task:v1:')
}
