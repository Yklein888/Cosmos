  ...
  return (
    <div className="space-y-3">
      {blocks.map((block, i) => {
        ...
        if (block.type === 'code') {
          return (
            <pre key={i} className="text-xs text-zinc-300 font-mono bg-zinc-900/60 rounded-lg p-4 leading-relaxed overflow-x-auto custom-scrollbar border border-cosmos-border/50">
              {block.content}
            </pre>
          )
        }
        if (block.type === 'hr') {
          return <hr key={i} className="border-cosmos-border" />
        }
        if (block.type === 'table' && block.headers) {
          return (
            <div key={i} className="overflow-x-auto rounded-xl border border-cosmos-border">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-zinc-800/60 border-b border-cosmos-border">
                ...
                <tbody>
                  {(block.tableRows || []).map((row, ri) => (
                    <tr key={ri} className="border-b border-cosmos-border/40 hover:bg-zinc-800/20 transition-colors">
                      {row.map((cell, ci) => (
...
function DataTable({ rows }: { rows: Record<string, unknown>[] }) {
  ...
  return (
    <div>
      <div className="overflow-x-auto rounded-xl border border-cosmos-border">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-zinc-800/60 border-b border-cosmos-border">
              {columns.map((col) => (
          ...
          <tbody>
            {displayRows.map((row, i) => (
              <tr key={i} className="border-b border-cosmos-border/40 hover:bg-zinc-800/20 transition-colors">
                {columns.map((col) => {
          ...
          if (isComplex) {
            return (
              <div key={key}>
                ...
                <div className="mt-1.5 pl-3 border-l-2 border-cosmos-border/50">
