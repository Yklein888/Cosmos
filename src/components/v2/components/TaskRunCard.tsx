...
export function StepResultCard({ result, label }: { result: WorkflowStepResult; label: string }) {
  ...
  return (
    <div className={`rounded-lg border ${
      isCompleted ? 'border-emerald-500/20' : isFailed ? 'border-red-500/20' : 'border-cosmos-border'
    }`}>
      <button
      ...
      {showOutput && hasOutput && (
        <div className="border-t border-cosmos-border px-2.5 py-2 max-h-64 overflow-y-auto custom-scrollbar select-text">
          {isFailed ? (
...
export function TaskRunCard({ run, index, nodeLabels }: TaskRunCardProps) {
  ...
  return (
    <div className="border border-cosmos-border rounded-lg overflow-hidden">
      <button
      ...
      {expanded && (
        <div className="border-t border-cosmos-border bg-zinc-900/50 px-3 py-2.5 space-y-2">
          {/* Summary */}
          {run.summary && (
