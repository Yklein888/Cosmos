...
export function GenerateTaskModal({ onClose }: Props) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setError(null)
    setProgress('Starting COSMOS session...')
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg mx-4 bg-cosmos-card border border-cosmos-border rounded-xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-cosmos-border">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-500">Describe your task and COSMOS will build it with a full workflow</p>
            </div>
          </div>
          {!isGenerating && (
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full px-3 py-2.5 bg-cosmos-bg border border-cosmos-border rounded-lg text-xs text-white placeholder-zinc-600 outline-none focus:border-cosmos-accent resize-none disabled:opacity-50"
              onKeyDown={(e) => {
              ...
              <div className="space-y-1.5">
                {SUGGESTIONS.map((s) => (
                  <button
                    key={s}
                    onClick={() => setPrompt(s)}
                    className="w-full text-left px-3 py-2 rounded-lg border border-cosmos-border hover:border-zinc-600 bg-cosmos-bg text-[11px] text-zinc-400 hover:text-white transition-colors truncate"
                  >
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-5 py-3 border-t border-cosmos-border bg-cosmos-bg/50">
          <span className="text-[10px] text-zinc-600">
            {isGenerating ? 'COSMOS is generating...' : 'Generates task + workflow'}
          </span>
