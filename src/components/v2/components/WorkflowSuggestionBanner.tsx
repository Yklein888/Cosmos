  ...
  return (
    <div className="mx-4 mt-2 flex items-center gap-3 px-4 py-2.5 rounded-lg border border-blue-500/20 bg-blue-500/5">
      ...
      <button
        ...
        disabled={!isPro}
        className={`px-3 py-1 rounded-lg text-xs font-medium border transition-colors whitespace-nowrap flex items-center gap-1.5 ${isPro ? 'text-blue-400 border-blue-500/30 hover:bg-blue-500/10' : 'text-zinc-500 border-cosmos-border cursor-not-allowed opacity-60'}`}
      >
        Save as Task
        {!isPro && <ProBadge />}
      </button>
