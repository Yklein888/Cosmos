...
export function AgentNode({ data, selected }: AgentNodeProps) {
  ...
  return (
    <div className="flex flex-col items-center">
      <Handle type="target" position={Position.Top} className="!w-2.5 !h-2.5 !bg-emerald-500 !border-2 !border-cosmos-bg" />

      <div
        className={`w-[200px] bg-cosmos-card border-2 rounded-lg transition-all ${
          selected
      ...
      <Handle type="source" position={Position.Bottom} className="!w-2.5 !h-2.5 !bg-emerald-500 !border-2 !border-cosmos-bg" />
    </div>
  )
}
