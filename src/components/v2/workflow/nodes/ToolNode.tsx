...
export function ToolNode({ data, selected }: ToolNodeProps) {
  ...
  return (
    <div className={`w-[180px] bg-cosmos-card border-2 rounded-lg transition-all ${border}`}>
      <Handle type="target" position={Position.Top} className="!w-2.5 !h-2.5 !bg-blue-500 !border-2 !border-cosmos-bg" />

      <div className="px-3 py-2.5">
        <div className="flex items-center gap-2 mb-1">
          <div className="w-6 h-6 rounded-md bg-cosmos-bg flex items-center justify-center flex-shrink-0">
      ...
      <Handle type="source" position={Position.Bottom} className="!w-2.5 !h-2.5 !bg-blue-500 !border-2 !border-cosmos-bg" />
    </div>
  )
}
