...
export function ResultsDisplayNode({ data, selected }: ResultsDisplayNodeProps) {
  ...
  return (
    <div className={`bg-cosmos-card border-2 rounded-lg transition-all ${border} ${hasData ? 'w-[260px]' : 'w-[180px]'}`}>
      <Handle type="target" position={Position.Top} className="!w-2.5 !h-2.5 !bg-cyan-500 !border-2 !border-cosmos-bg" />
      ...
      <Handle type="source" position={Position.Bottom} className="!w-2.5 !h-2.5 !bg-cyan-500 !border-2 !border-cosmos-bg" />
    </div>
  )
}
