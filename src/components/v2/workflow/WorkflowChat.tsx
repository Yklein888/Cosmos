        ...
        <div className="px-3 py-2.5 rounded-lg bg-red-500/5 border border-red-500/20">
          ...
          <div className="flex items-center gap-2 mt-2 pt-2 border-t border-red-500/10">
            ...
            {onSimplify && (
              <button
                onClick={onSimplify}
                className="flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-medium text-zinc-400 border border-cosmos-border hover:text-white hover:border-zinc-600 transition-colors"
              >
    ...
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-2`}>
      <div className={`max-w-[90%] px-3 py-2 rounded-lg text-xs leading-relaxed ${
        isUser
          ...
          : 'bg-cosmos-card border border-cosmos-border text-zinc-300'
      }`}>
...
export function WorkflowChat() {
  ...
  return (
    <div className="w-72 border-r border-cosmos-border flex flex-col bg-cosmos-bg flex-shrink-0">
      {/* Header */}
      <div className="px-3 py-2.5 border-b border-cosmos-border flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-3 py-2">
        {isEmpty && (
          <div className="mt-4 mb-2">
            <div className="text-center mb-5">
              ...
              <p className="text-[10px] text-zinc-500 leading-relaxed">Describe your task in plain English.<br />COSMOS will build the workflow.</p>
            </div>
            <div className="space-y-1.5">
              {SUGGESTIONS.map((s, i) => (
                <button
                  ...
                  className="w-full text-left flex items-start gap-2 px-3 py-2.5 rounded-lg border border-cosmos-border bg-cosmos-card text-[10px] text-zinc-400 hover:text-white hover:border-zinc-600 transition-colors leading-relaxed"
                >
        ...
        {chatIsGenerating && (
          <div className="mb-2">
            <div className="px-3 py-2.5 rounded-lg bg-cosmos-card border border-purple-500/20">
      ...
      {nodes.length > 1 && !isEmpty && !chatIsGenerating && (
        <div className="px-3 py-1.5 border-t border-cosmos-border flex-shrink-0">
          <button
      ...
      {/* Input area */}
      <div className="px-3 py-2.5 border-t border-cosmos-border flex-shrink-0">
        <div className="flex gap-1.5">
          <textarea
            ...
            className="flex-1 bg-cosmos-card border border-cosmos-border rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-zinc-600 outline-none focus:border-blue-500 resize-none disabled:opacity-50"
          />
