...
export function WorkflowTemplatesModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="bg-cosmos-card border border-cosmos-border rounded-xl shadow-2xl w-[520px] max-h-[80vh] overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-5 py-4 border-b border-cosmos-border flex items-center justify-between">
          <div>
        ...
        <div className="p-4 space-y-3 overflow-y-auto custom-scrollbar max-h-[60vh]">
          {TEMPLATES.map((t) => (
            <button
              ...
              className="w-full text-left p-4 bg-cosmos-bg border border-cosmos-border rounded-lg hover:border-blue-500/30 hover:bg-blue-600/5 transition-colors group"
            >
        ...
        {/* Footer */}
        <div className="px-5 py-3 border-t border-cosmos-border">
