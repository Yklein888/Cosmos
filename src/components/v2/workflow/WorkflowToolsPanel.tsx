...
function StructuralNodeCard({ node }: { node: typeof STRUCTURAL_NODES[number] }) {
  ...
  return (
    <div
      ...
      className="flex items-center gap-2.5 px-3 py-2 rounded-lg border border-cosmos-border bg-cosmos-card hover:border-zinc-600 cursor-grab active:cursor-grabbing transition-colors group"
    >
...
export function WorkflowToolsPanel() {
  ...
  return (
    <div className="w-64 border-r border-cosmos-border flex flex-col bg-cosmos-bg flex-shrink-0">
      {/* Header */}
      <div className="px-4 py-3 border-b border-cosmos-border flex-shrink-0">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-7 pr-3 py-1.5 bg-cosmos-card border border-cosmos-border rounded-lg text-xs text-white placeholder-zinc-600 outline-none focus:border-cosmos-accent"
          />
        </div>
      </div>

      {/* Filter tabs */}
      <div className="flex gap-1 px-4 py-2 border-b border-cosmos-border flex-shrink-0">
