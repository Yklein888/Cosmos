        ...
        {selectedId === 'custom' && (
          <div className="mb-6">
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2.5 bg-cosmos-bg border border-cosmos-border rounded-lg text-xs text-white placeholder-zinc-600 outline-none focus:border-blue-500 resize-none"
            />
          </div>
        )}
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-8">
      <div className="w-full max-w-md">
        ...
        <div className="space-y-3 mb-8">
          {/* Jira — Pro only */}
          <div className={`rounded-xl border overflow-hidden ${jiraConnected ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-cosmos-card border-cosmos-border'}`}>
          ...
          {/* GitHub — all tiers */}
          <div className={`rounded-xl border overflow-hidden ${githubConnected ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-cosmos-card border-cosmos-border'}`}>
            <div className="flex items-center gap-3 p-4">
          ...
          {/* Slack — all tiers */}
          <div className={`rounded-xl border overflow-hidden ${slackConnected ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-cosmos-card border-cosmos-border'}`}>
            <div className="flex items-center gap-3 p-4">
              <div className="w-10 h-10 rounded-lg bg-zinc-800 flex items-center justify-center flex-shrink-0">
              ...
              <div className="border-t border-cosmos-border px-4 pb-4 pt-3 space-y-3">
                ...
                <div className="space-y-2">
                  <input
                    ...
                    className="w-full bg-zinc-900 border border-cosmos-border rounded-lg px-3 py-1.5 text-xs text-white placeholder-zinc-600 focus:outline-none focus:border-zinc-500 font-mono"
                  />
                  <div className="flex gap-2">
                    <input
                      type="text"
            ...
            return (
              <div
                ...
                className={`rounded-xl border transition-all ${
                  task.enabled ? 'bg-cosmos-card border-cosmos-border' : 'bg-cosmos-bg border-cosmos-border/50 opacity-50'
                }`}
              >
                <div className="flex items-center gap-3 p-4">
                  <button
                ...
                {isExpanded && task.summary.length > 0 && (
                  <div className="px-4 pb-4 pl-12">
                    <div className="p-3 rounded-lg bg-cosmos-bg border border-cosmos-border/50">
        ...
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold transition-colors ${
              ...
              'bg-cosmos-card border border-cosmos-border text-zinc-600'
            }`}>
              {s < step ? <Icon icon="lucide:check" className="text-xs" /> : s}
            </div>
            {s < 4 && <div className={`w-12 h-0.5 rounded ${s < step ? 'bg-emerald-500' : 'bg-cosmos-border'}`} />}
          </div>
        ))}
      </div>
