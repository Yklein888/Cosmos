    ...
    <div className="flex items-center gap-3 mb-6">
      <div className="relative flex-1">
        ...
        <input
          ...
          className="w-full pl-8 pr-3 py-1.5 bg-zinc-800/80 border border-cosmos-border rounded-lg text-xs text-white placeholder-zinc-600 outline-none focus:border-blue-500 transition-colors"
        />
      </div>
      <select
        value={filter.taskId || ''}
        onChange={(e) => onChange({ ...filter, taskId: e.target.value || null })}
        className="bg-zinc-800/80 border border-cosmos-border rounded-lg px-2.5 py-1.5 text-xs text-zinc-300 outline-none focus:border-blue-500 cursor-pointer"
      >
...
function ResultFeedCard({ run }: { run: FlattenedRun }) {
  ...
  return (
    <div className="border border-cosmos-border rounded-xl overflow-hidden hover:border-zinc-600/50 transition-colors">
      <button
      ...
      {expanded && (
        <div className="border-t border-cosmos-border bg-zinc-900/30 px-4 py-3 space-y-4">
          {/* Structured Result */}
          {structuredResult && (
            <div>
              ...
              <div className="rounded-lg border border-cosmos-border/50 bg-zinc-900/40 p-3 max-h-[300px] overflow-y-auto custom-scrollbar select-text">
          ...
          {run.logs && run.logs.length > 0 && (
            <details className="pt-1 border-t border-cosmos-border">
    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar">
      <div className="p-6">
        ...
        {latestStructuredResult && (
          <div className="mb-6 rounded-xl border border-cosmos-border overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3 bg-zinc-800/40 border-b border-cosmos-border">
