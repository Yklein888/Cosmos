...
function TaskRow({ task, selected, onClick }: { task: Task; selected: boolean; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-center gap-4 px-4 py-3 border-b border-cosmos-border transition-colors cursor-pointer hover:bg-zinc-800/30 ${
    ...
    >
      ...
      <div className="w-16 flex items-center gap-1">
        {hasJira && (
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] border border-cosmos-border bg-cosmos-card">
...
export default function TasksPage() {
  ...
  const handleImportClipboard = useCallback(async () => {
    ...
    try {
      ...
      if (!isPilosClipboardString(text)) {
        setImportError('Clipboard does not contain a valid COSMOS task')
        return
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('cosmos:new-task', handler)
    return () => window.removeEventListener('cosmos:new-task', handler)
  }, [setShowCreateModal])
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Filters */}
        <div className="flex items-center gap-3 px-6 py-3 border-b border-cosmos-border flex-shrink-0">
          <div className="flex items-center gap-2 flex-1">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 bg-cosmos-card border border-cosmos-border rounded-lg text-xs text-white placeholder-zinc-600 outline-none focus:border-cosmos-accent w-48"
              />
            </div>
            <select
              value={filter.status}
              onChange={(e) => setFilter({ status: e.target.value as TaskStatus | 'all' })}
              className="px-3 py-1.5 bg-cosmos-card border border-cosmos-border rounded-lg text-xs text-zinc-400 outline-none appearance-none"
            >
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              disabled={!isPro}
              className={`px-3 py-1.5 bg-cosmos-card border border-cosmos-border text-xs font-medium rounded-lg transition-all flex items-center gap-1.5 ${isPro ? 'hover:border-zinc-600 text-zinc-300 hover:text-white' : 'opacity-50 cursor-not-allowed text-zinc-500'}`}
            >
            ...
            {showImportMenu && isPro && (
              <div className="absolute top-full right-0 mt-1 bg-zinc-800 border border-cosmos-border rounded-lg shadow-xl z-10 overflow-hidden w-52">
                <button
                ...
                >
                  ...
                  <span className="text-xs text-white">Import from .COSMOS file</span>
                </button>
                <button
        ...
        {/* Table Header */}
        <div className="flex items-center gap-4 px-4 py-2 border-b border-cosmos-border bg-cosmos-card/30 flex-shrink-0">
