      ...
      {data.map((d) => {
        ...
        return (
          <div key={d.date} className="flex-1 flex flex-col items-center group relative">
            ...
            {/* Tooltip */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full hidden group-hover:block bg-zinc-800 border border-cosmos-border rounded-lg px-2 py-1 whitespace-nowrap z-10">
...
function ActivityTable({ entries }: { entries: ReturnType<typeof useAnalyticsStore.getState>['entries'] }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-4 px-4 py-2 border-b border-cosmos-border">
      ...
      {entries.map((entry) => (
        <div key={entry.id} className="table-row-hover flex items-center gap-4 px-4 py-2.5 border-b border-cosmos-border/50 transition-colors">
      ...
      <div className="p-6">
        ...
        <div>
          <div className="bg-cosmos-card border border-cosmos-border rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-cosmos-border">
